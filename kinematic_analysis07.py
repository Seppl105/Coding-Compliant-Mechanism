...
t32 = t31 + np.deg2rad( angle_deg(sys2_links[2]) - angle_deg(sys1_links[2]) ) #################################
...
l_A1A2, t_A1A2 = get_A1A2(sys1_links[1], sys2_links[1])
...
k1_x, k1_y, k2_x, k2_y, l_c1 = compute_k_offsets(sys1_links, sys2_links, coupler_links[0], Phi2)
...
##########################################
##########################################
eqs_sys2.append( r21*sp.cos(t21) + l_A1A2*sp.cos(t_A1A2) - r22*sp.cos(t22 + Phi2) + ( k2_x*sp.cos(Phi2) - k2_y*sp.sin(Phi2) ) - l_c1*sp.cos(alpha1) - ( r41*(1)*sp.cos(t41) + k1_x )) # note that [k2_x, k2_y] is converted using relations knowen from polar coordinates 
eqs_sys2.append( r21*sp.sin(t21) + l_A1A2*sp.sin(t_A1A2) - r22*sp.sin(t22 + Phi2) + ( k2_x*sp.sin(Phi2) + k2_y*sp.cos(Phi2) ) - l_c1*sp.sin(alpha1) - ( r41*(1)*sp.sin(t41) + k1_y )) # note that [k2_x, k2_y] is converted using relations knowen from polar coordinates
...
# ]
x0_sys2 = [
    math.radians(angle_deg(sys2_links[0])) - Phi2,    # t12
    math.radians(angle_deg(sys2_links[1])) - Phi2,  # t22
    math.radians(angle_deg(sys2_links[3])) - Phi2,   # t42
...
B2_global   = O2_2_global + R(Phi2) @ to_np(B2_loc) 
...
print("local: t12 t22 t32 t42: ", np.rad2deg(t12_val  + Phi2), np.rad2deg(t22_val + Phi2), np.rad2deg(t32_val + Phi2), np.rad2deg(t42_val + Phi2))
