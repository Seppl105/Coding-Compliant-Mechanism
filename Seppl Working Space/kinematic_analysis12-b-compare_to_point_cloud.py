...
# --- 11.4b Build corresponding point sets for rigid fit ---
def rigid_fit_2d(P, Q):
    """
    Find rotation R and translation t that best map
    P[i] -> Q[i] in least-squares sense.

    P, Q: (N, 2) arrays of corresponding points
    Returns:
...
# Use all ground joints as a robust reference (you can add A/B joints too if you like)
P_model = np.vstack([
    pts12_ver["O2_1"], pts12_ver["O4_1"],
    pts12_ver["O2_2"], pts12_ver["O4_2"],
    pts3_ver["O2_3"],  pts3_ver["O4_3"],
    pts4_ver["O2_4"],  pts4_ver["O4_4"],
])

Q_meas = np.vstack([
    joints_new[0]["O2"], joints_new[0]["O4"],
    joints_new[1]["O2"], joints_new[1]["O4"],
    joints_new[2]["O2"], joints_new[2]["O4"],
    joints_new[3]["O2"], joints_new[3]["O4"],
])

R_fit, t_fit = rigid_fit_2d(P_model, Q_meas)

def apply_rigid(p):
    """Apply best-fit rotation+translation to model point p."""
    p = np.asarray(p, float)
    return R_fit @ p + t_fit
...
pairs += [
    (apply_rigid(pts12_ver["O2_1"]), joints_new[0]["O2"]),
    (apply_rigid(pts12_ver["O4_1"]), joints_new[0]["O4"]),
    (apply_rigid(pts12_ver["A1"]),   joints_new[0]["A"]),
    (apply_rigid(pts12_ver["B1"]),   joints_new[0]["B"]),
]

# mech 2: O2_2, O4_2, A2, B2
pairs += [
    (apply_rigid(pts12_ver["O2_2"]), joints_new[1]["O2"]),
    (apply_rigid(pts12_ver["O4_2"]), joints_new[1]["O4"]),
    (apply_rigid(pts12_ver["A2"]),   joints_new[1]["A"]),
    (apply_rigid(pts12_ver["B2"]),   joints_new[1]["B"]),
]

# mech 3: O2_3, O4_3, A3, B3
pairs += [
    (apply_rigid(pts3_ver["O2_3"]), joints_new[2]["O2"]),
    (apply_rigid(pts3_ver["O4_3"]), joints_new[2]["O4"]),
    (apply_rigid(pts3_ver["A3"]),   joints_new[2]["A"]),
    (apply_rigid(pts3_ver["B3"]),   joints_new[2]["B"]),
]

# mech 4: O2_4, O4_4, A4, B4
pairs += [
    (apply_rigid(pts4_ver["O2_4"]), joints_new[3]["O2"]),
    (apply_rigid(pts4_ver["O4_4"]), joints_new[3]["O4"]),
    (apply_rigid(pts4_ver["A4"]),   joints_new[3]["A"]),
    (apply_rigid(pts4_ver["B4"]),   joints_new[3]["B"]),
]
...
# System 1 model
plot_segment(apply_rigid(pts12_ver["O2_1"]), apply_rigid(pts12_ver["O4_1"]))
plot_segment(apply_rigid(pts12_ver["O2_1"]), apply_rigid(pts12_ver["A1"]))
plot_segment(apply_rigid(pts12_ver["O4_1"]), apply_rigid(pts12_ver["B1"]))
plot_segment_dashed(apply_rigid(pts12_ver["A1"]), apply_rigid(pts12_ver["B1"]))

# System 2 model
plot_segment(apply_rigid(pts12_ver["O2_2"]), apply_rigid(pts12_ver["O4_2"]))
plot_segment(apply_rigid(pts12_ver["O2_2"]), apply_rigid(pts12_ver["A2"]))
plot_segment(apply_rigid(pts12_ver["O4_2"]), apply_rigid(pts12_ver["B2"]))
plot_segment_dashed(apply_rigid(pts12_ver["A2"]), apply_rigid(pts12_ver["B2"]))

# System 3 model
plot_segment(apply_rigid(pts3_ver["O2_3"]), apply_rigid(pts3_ver["O4_3"]))
plot_segment(apply_rigid(pts3_ver["O2_3"]), apply_rigid(pts3_ver["A3"]))
plot_segment(apply_rigid(pts3_ver["O4_3"]), apply_rigid(pts3_ver["B3"]))
plot_segment_dashed(apply_rigid(pts3_ver["A3"]), apply_rigid(pts3_ver["B3"]))

# System 4 model
plot_segment(apply_rigid(pts4_ver["O2_4"]), apply_rigid(pts4_ver["O4_4"]))
plot_segment(apply_rigid(pts4_ver["O2_4"]), apply_rigid(pts4_ver["A4"]))
plot_segment(apply_rigid(pts4_ver["O4_4"]), apply_rigid(pts4_ver["B4"]))
plot_segment_dashed(apply_rigid(pts4_ver["A4"]), apply_rigid(pts4_ver["B4"]))
